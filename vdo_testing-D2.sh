...
benchmark()
{
	...
	then
		...
		log_files_list=$log_files_list" ./dedisbench2/$run_ID"

		cd dedisbenchv2
		./DEDISbench -$test_type -p -t20 -f$input_file >> "../logs/dedisbench2/$run_ID"
		cd ..
	else
...
run_all_tests()
{
	echo "main: Starting all tests"
	
	for benchmark in dedisbench2
	do
